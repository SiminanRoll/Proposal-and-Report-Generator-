...
test("v1.8.5 gives Project Coverage cards stronger premium metric hierarchy", () => {
...
test("v1.8.5 hardware inventory is fixed-width to the presentation viewport without horizontal scrolling", () => {
...
test("v1.8.5 uses client-facing Technology Health wording instead of internal weighting language", () => {
  assert.match(exportHtml, /critical systems need attention/);
  assert.doesNotMatch(exportHtml, /critical systems weighted/);
});

test("v1.8.5 Captain's Log creation uses the shared Supabase app_events queue", () => {
