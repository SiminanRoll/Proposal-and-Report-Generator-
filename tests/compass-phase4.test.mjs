...
async function transpileModule(relativePath) {
  ...
  const file = path.join(os.tmpdir(), `client-compass-phase4-${path.basename(relativePath).replace(/\W/g, "-")}-${Date.now()}-${Math.random().toString(36).slice(2)}.mjs`);
...
test("Phase 4 release is versioned as Client Compass 1.4.7", () => {
  ...
  assert.match(version, /APP_VERSION = "1\.4\.7"/);
});
