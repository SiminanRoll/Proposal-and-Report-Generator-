...
`src/lib/compass` owns the Ninja master spreadsheet header mapping, explicit organization resolution, normalized client/location/device model, physical-versus-virtual classification, current findings, Compass Priority Score, opportunity valuation, named-location snapshots, grouped project packaging, configuration fingerprinting, automatic recalculation, and browser-local IndexedDB persistence for large current-state datasets.

Only one committed technical dataset is stored. A later import replaces the device and finding snapshot while preserving manually maintained client workflow fields and Review Outcome decisions. Card queues and client workspaces are projections of that one current snapshot; changing criteria or estimate assumptions updates the projection without creating history. Reviews Due and Quote Needed are workflow projections only and do not contribute to the technical priority score. No inventory history, score history, trends, analytics, backend, or external data transmission are introduced.

### Multisite and project packaging

`src/lib/compass/project-packaging.ts` derives reportable named-location snapshots and practical project packages from the same committed devices, findings, opportunities, configuration, and Review Outcome used by Compass. Generic location placeholders are omitted from client-facing location views without removing their devices from authoritative inventory. Review Outcome decisions are applied first, and a claimed-device/one-time allowance model prevents overlapping findings from duplicating replacement value. The resulting packages carry devices, locations, technical drivers, disposition, client and Advantage responsibilities, timing, quote status, assumptions, inclusion status, source, and estimated value into the workspace, generator, presentation, and PDF.

### Managed-client generator bridge

`src/lib/compass/generator-bridge.ts` translates a selected client’s current committed Compass devices, location snapshots, findings, and grouped project packages into the facts used by the client-report generator. The current Ninja/Client Compass snapshot is authoritative for inventory identity, names, locations, device classes, operating systems, check-in status, and complete counts. An optional ScalePad PDF can safely enrich matching devices with age, purchase date, warranty, and aggregate lifecycle summaries without replacing authoritative inventory rows. Source totals and device classes are reconciled before publication; incomplete authoritative inventory creates a blocking review exception. The connection remains browser-local, keeps Huntress as the required current security source, and can be refreshed inside an existing report workspace after a newer import. Proposal workflows continue to use the RFT as their primary technical source.
