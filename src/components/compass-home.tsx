...
import { cardMetrics } from "@/lib/compass/engine";
import { useCompassState } from "@/lib/compass/store";
import type { CompassCardCategory, CompassCardMetric } from "@/lib/compass/types";
...
const cardDefinitions: CardDefinition[] = [
  { id: "all", title: "Clients Needing Projects", countLabel: "clients with a current project opportunity", valueLabel: "deduplicated estimated opportunity represented", description: "A current snapshot of organizations with server, workstation, operating-system, or capacity needs.", accent: "blue", icon: "compass" },
  { id: "critical-server", title: "Critical Server Projects", countLabel: "clients needing immediate server attention", valueLabel: "estimated immediate server value", description: "Unsupported server operating systems and physical servers substantially beyond expected lifecycle.", accent: "red", icon: "server" },
  { id: "server-planning", title: "Server Planning", countLabel: "clients approaching a server project", valueLabel: "estimated planned server value", description: "Windows Server 2016 and aging server platforms that should enter the planning conversation now.", accent: "amber", icon: "calendar" },
  { id: "windows-10", title: "Windows 10 Refresh", countLabel: "clients with Windows 10 exposure", valueLabel: "estimated refresh value", description: "Organizations with Windows 10 devices that need replacement or operating-system modernization.", accent: "cyan", icon: "windows" },
  { id: "workstation-lifecycle", title: "Workstation Lifecycle", countLabel: "clients with replacement-ready workstations", valueLabel: "estimated workstation value", description: "Replace Now and Plan Soon physical devices grouped into practical workstation modernization opportunities.", accent: "violet", icon: "workstation" },
  { id: "storage", title: "Storage Attention", countLabel: "clients with capacity concerns", valueLabel: "estimated remediation value", description: "Critical and watch-level system drives, server volumes, and capacity constraints that merit a focused conversation.", accent: "teal", icon: "storage" },
];

function OpportunityIcon({ type, ...props }: SVGProps<SVGSVGElement> & { type: CardDefinition["icon"] }) {
...
export function CompassHome() {
  ...
  const metrics = useMemo(() => cardMetrics(dataset), [dataset]);
    ...
    <div className="compass-home">
      <section className="compass-intro" aria-labelledby="compass-title">
        ...
        <div className="compass-intro-actions">
          <span className={`compass-preview-badge${dataset ? " is-live" : ""}`}>{dataset ? formatRefresh(dataset.importedAt) : "Live data required"}</span>
      ...
      {!dataset && (
        <section className="compass-empty-state" aria-label="Client Compass data required">
          ...
          <div><strong>Import the current Ninja master spreadsheet</strong><p>The six opportunity cards will calculate from committed client and device data. No illustrative client counts or values are being shown.</p></div>
            ...
            <article key={card.id} className={`compass-opportunity-card accent-${card.accent}${isFlipped ? " is-flipped" : ""}`}>
              <div className="compass-card-inner">
                ...
                <div className="compass-card-face compass-card-back" aria-hidden={!isFlipped}>
                  ...
                  <span className="compass-card-estimate-note">Internal opportunity estimate · editable demo assumptions · not a client quote</span>
      ...
      <footer className="compass-footnote">
        ...
        <span>Priority score and estimates are explainable, configurable, and current-state only.</span>
      </footer>
      ...
      {activeCard && (
        <div className="compass-drawer-backdrop" role="presentation" onMouseDown={() => setActiveCard(null)}>
          <aside className="compass-client-drawer" role="dialog" aria-modal="true" aria-labelledby="compass-drawer-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="compass-demo-notice">This Phase 2 queue shows committed calculations only. Full filtering, workflow controls, and client workspaces remain reserved for Phase 3.</div>
