...
export function CaptainsLogCloudSettings() {
  ...
  useEffect(() => {
    ...
    setRemembered(snapshot.signedIn);
    ...
    setStatus("Saved sign-in found. Restoring Supabase access…");
    void verifyCaptainsLogTaskConnection()
      ...
      .catch((cause) => {
        ...
        setRemembered(current.signedIn);
        setConnected(false);
        setStatus(current.signedIn
  ...
  const connect = async () => {
    ...
    try {
      ...
      setRemembered(getCaptainsLogCloudAuthSnapshot().signedIn);
  ...
  const retrySavedSession = async () => {
    ...
    if (!snapshot.signedIn) {
      setRemembered(false);
      setConnected(false);
      setStatus("Supabase sign-in is required.");
      return;
    }
    ...
    <div className="compass-settings-cloud-stack">
      <div className="compass-settings-subpanel captains-log-cloud-settings">
        ...
        <div className="compass-settings-grid two-column captains-log-cloud-grid">
          ...
          <label className="compass-settings-field"><span>Password</span><input type="password" value={password} disabled={remembered} placeholder={remembered ? "Remembered on this device" : "Password"} autoComplete="current-password" onChange={(event) => setPassword(event.target.value)}/></label>
        </div>
        <p className="captains-log-cloud-remember-note">Remember this device is automatic in this browser. Compass keeps a renewable Supabase session locally and refreshes it in the background; your password is never saved.</p>
