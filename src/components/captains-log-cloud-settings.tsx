...
export function CaptainsLogCloudSettings() {
  ...
  useEffect(() => {
    ...
    setStatus(snapshot.signedIn
      ? `Supabase connected as ${snapshot.email} · historical task and activity data can be read directly`
      : snapshot.configured ? "Configuration saved · sign in to connect" : "Not configured");
  }, []);
  ...
  const connect = async () => {
    ...
    try {
      ...
      setConnected(ready);
      setStatus(ready
        ? `Supabase connected as ${snapshot.email} · historical task and activity data are available`
        : "Signed in, but the Supabase history tables could not be read.");
    } catch (cause) {
      setConnected(false);
  ...
  return <section className="compass-settings-section captains-log-cloud-settings">
    <div className="compass-settings-section-heading"><div><span className="compass-kicker">Supabase history</span><h2>History connection</h2><p>Client Compass reads shared task and Call Mode history directly from Supabase. This connection supplies client matches, contacts, account-review activity, recent activity, and open/planned work.</p></div><span className={`captains-log-cloud-badge${connected ? " is-connected" : ""}`}>{connected ? "Connected" : "Not connected"}</span></div>
