...
const THRESHOLD_FIELDS: Array<[keyof CompassConfig["thresholds"], string, string, string]> = [
  ...
  ["workstationPlanSoonYears", "Workstation planning age", "years", "Age where a workstation moves into planning."],
  ["workstationReplaceNowYears", "Workstation replacement age", "years", "Age where a workstation becomes a replacement signal."],
...
export function CompassPricingSettingsPage() {
  ...
  return <div className="compass-admin-page compass-settings-page compass-settings-page-modern compass-pricing-settings-page">
    ...
    <section className="compass-settings-section settings-number-section" id="pricing-lifecycle">
      <div className="compass-settings-section-heading"><div><span className="compass-settings-section-kicker">Lifecycle signals</span><h2>Lifecycle thresholds</h2><p>These values determine when hardware and storage move from current to planning or critical attention.</p></div><span className="settings-pricing-scope-badge">Recalculates Compass</span></div>
      <div className="settings-number-card-grid">{THRESHOLD_FIELDS.map(([key, label, unit, detail]) => <label className="settings-number-card" key={key}><span><strong>{label}</strong><small>{detail}</small></span><div className="compass-settings-number is-suffix"><input inputMode="numeric" type="number" min="0" step="1" value={draft.thresholds[key]} onChange={(event) => setDraft((current) => ({ ...current, thresholds: { ...current.thresholds, [key]: Math.max(0, Number(event.target.value) || 0) } }))}/><em>{unit}</em></div></label>)}</div>
    </section>
    ...
    <footer className="compass-settings-savebar compass-settings-savebar-clean settings-detail-savebar"><span>Lifecycle and project-estimate changes are saved together and immediately recalculate the current Compass snapshot.</span><button className="button primary" type="button" disabled={!ready || saving} onClick={() => void savePlanning()}>{saving ? "Saving…" : "Save planning numbers"}</button></footer>
  </div>;
}
