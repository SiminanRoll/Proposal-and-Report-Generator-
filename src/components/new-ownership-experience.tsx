...
function TechnologyHealthSlide({ project }: { project: Project }) {
  ...
  return <div className={styles.healthSlide}>
    ...
    <div className={`${styles.metrics} ${styles.metricsFour}`}>
      ...
      <article><strong>{networkDevices || "—"}</strong><span>Network devices</span><small>Managed infrastructure included in this review</small></article>
    </div>
...
function RecapSlide({ project }: { project: Project }) {
  ...
  return <div className={styles.recapSlide}>
    ...
    <div className={styles.recapCards}>
      ...
      <article><b>YOUR TECHNOLOGY</b><strong>{agingCount ? `${agingCount} aging system${agingCount === 1 ? " carries" : "s carry"} increased risk of failure` : "A healthy lifecycle baseline"}</strong><p>{agingCount ? "As equipment ages, the risk of unexpected failure increases. Advantage can help prioritize the highest-risk systems and build a practical plan for what comes next." : "The current lifecycle data does not identify an aging-system concern that needs to dominate the transition."}</p></article>
