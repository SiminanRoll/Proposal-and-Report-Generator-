...
export function A360ConversationWorkspace({ project, onUpdate }: { project: Project; onUpdate: (project: Project) => void }) {
  ...
  function applyTailoredOutput() {
    ...
    const report = parseA360TailoredReport(tailoredOutput, record.report);
    onUpdate({ ...project, a360Conversation: { ...record, report }, presentation: { ...project.presentation, title: report.title, executiveSummary: report.executiveSummary } });
  ...
  return <main className="workspace-page a360-record-workspace">
    ...
    <div className="record-grid">
      <section className="record-card"><h2>Client-facing report copy</h2><p>Edit anything below before opening the PDF. Changes save with this workspace.</p>{legacyDefaultCopy ? <p className="legacy-note">This workspace has older default A360 wording saved in it. Opening the PDF will refresh that default copy automatically, or you can use <strong>Use latest A360 recap</strong> now.</p> : null}<div className="report-editor">
        ...
        <label><span>Next step</span><textarea value={record.report.nextStepSummary} onChange={(event) => updateReport("nextStepSummary", event.target.value)} /></label>
      </div>
      ...
      <section className="record-card" style={{marginTop:18}}><h2>Next step</h2><p>{formatPlanningAppointment(record.appointment)}</p><div className="record-fact"><small>Technology Consultant</small><strong>{record.appointment.consultantName}</strong></div><p className="record-note">The exported report deliberately avoids internal sales workflow language and does not state that the environment, security posture, risks, or recommendations have already been verified.</p></section></aside>
    </div>
  </main>;
}
