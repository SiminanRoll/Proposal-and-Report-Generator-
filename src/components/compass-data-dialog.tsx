...
export function CompassDataDialog({ open, dataset, config, onClose, onCommitted }: Props) {
  ...
  const closeDialog = () => { setParsed(null); setResolutions({}); setError(""); onClose(); };
  useEffect(() => {
    if (!open) return;
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") { setParsed(null); setResolutions({}); setError(""); onClose(); }
    };
  ...
  const commit = () => {
    ...
    saveCompassDataset(preview.dataset);
    onCommitted();
    closeDialog();
  };
  ...
  return (
    <div className="compass-modal-backdrop" role="presentation" onMouseDown={closeDialog}>
      <section className="compass-modal compass-import-modal" role="dialog" aria-modal="true" aria-labelledby="compass-import-title" onMouseDown={(event) => event.stopPropagation()}>
        <header className="compass-modal-header">
          ...
          <button className="compass-drawer-close" type="button" onClick={closeDialog} aria-label="Close data import">×</button>
        </header>
        ...
        <label className="compass-file-drop">
          ...
          <strong>{busy ? "Reading spreadsheet…" : parsed ? parsed.sourceName : "Choose Ninja master spreadsheet"}</strong>
          <span>Supported: XLSX, XLS, XLSM, XLSB, CSV, and TSV. Processing stays in this browser.</span>
        </label>
        {error && <div className="compass-import-error">{error}</div>}
        ...
        <footer className="compass-modal-actions">
          ...
          <button className="button primary" type="button" disabled={!preview?.dataset || busy} onClick={commit}>{preview?.unresolvedOrganizations.length ? `Resolve ${preview.unresolvedOrganizations.length} organization${preview.unresolvedOrganizations.length === 1 ? "" : "s"}` : "Commit current snapshot"}</button>
        </footer>
      </section>
    </div>
  );
