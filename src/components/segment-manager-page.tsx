      ...
      return <article className={`segment-flip-card${isFlipped ? " is-flipped" : ""}`} key={snapshot.segment.id} style={{ "--segment-color": snapshot.segment.color } as CSSProperties}>
        <div className="segment-flip-inner">
          ...
          <section className="segment-card-face segment-card-back">
            ...
            <div className="segment-card-actions"><Link href={`/segments/${encodeURIComponent(snapshot.segment.id)}/`}>View clients</Link><button type="button" onClick={() => setEditing(snapshot.segment)}>Edit segment</button><button className="is-danger" type="button" onClick={() => { if (window.confirm(`Delete ${snapshot.segment.title}?`)) deleteSegment(snapshot.segment.id); }}>Delete</button></div>
          </section>
        </div>
      </article>;
    })}</section>}
