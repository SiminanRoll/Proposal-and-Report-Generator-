...
function DeviceDetailPresentation({ project }: { project: Project }) {
  const [filter, setFilter] = useState<"all" | "current" | "due-soon" | "overdue" | "storage" | "os">("all");
  ...
  const filterLabel = filter === "all" ? "all assets" : filter === "current" ? "healthy assets" : filter === "due-soon" ? "plan-soon assets" : filter === "overdue" ? "replace-now assets" : filter === "os" ? osSupport.attention ? "operating-system concerns" : "assets with reported operating systems" : storage.attention ? "storage-attention assets" : "assets with reported storage";
  const cards = [
