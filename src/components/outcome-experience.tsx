    ...
    <div className="presentation-health-cover">
      <div className="health-cover-main">
        ...
        <div className={`health-score-card-grid ${project.hipaa.enabled ? "" : "without-hipaa"}`}>
          ...
          <HealthScoreCard score={scores.planning} label="Resilience & planning" detail="Action coverage and replacement readiness" className="planning" />
        </div>
      </div>
...
function PlanPresentation({ project }: { project: Project }) {
  ...
  const incidents = factNumber(project, "huntress.incidentsReported");
  const investigated = factNumber(project, "huntress.signalsInvestigated");
  return <div className="presentation-section-layout client-action-plan"><div className="presentation-section-heading"><span className="presentation-kicker">Planning</span><h2>Turn the review into clear next steps.</h2><p>{project.hipaa.enabled ? "The roadmap below is generated from the actual replacement list, HIPAA readiness result, and security activity reviewed in this package." : "The roadmap below is generated from the actual replacement list, lifecycle health, and security activity reviewed in this package."}</p></div><div className="planning-context-strip"><span className={lifecycle.overdue ? "risk" : "healthy"}><strong>{lifecycle.overdue}</strong> replace now</span>{project.hipaa.enabled && <span className={hipaa.notYetAssessedCount ? "attention" : "healthy"}><strong>{hipaa.overall}/100</strong> HIPAA readiness</span>}<span className={incidents || investigated ? "attention" : "healthy"}><strong>{incidents + investigated}</strong> security follow-ups</span></div><div className="presentation-plan action-plan-grid">{actions.map((item, index) => <article className={`plan-action-${item.tone}`} key={item.id}><b>{String(index + 1).padStart(2, "0")}</b><div><div className="plan-action-meta"><span>{item.timing}</span><span>{item.owner}</span></div><h3>{item.title}</h3><p>{item.detail}</p></div></article>)}</div></div>;
}
