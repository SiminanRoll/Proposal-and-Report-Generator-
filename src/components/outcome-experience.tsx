  ...
  return <div className={`presentation-section-layout client-action-plan ${hasActionItems ? "action-mode" : "healthy-mode"}`}>
    <div className="planning-hero-grid">
      ...
      {noActionNeeded ? <section className="planning-consultation-banner">
        <div><span className="presentation-kicker">Recommended next step</span><h3>No immediate action needed</h3><p>Continue normal monitoring, maintenance, security protection, and support, then revisit the environment at the next scheduled review.</p></div>
        <div className="planning-session-outcomes">{["Environment healthy", "No open priorities", "Continue monitoring", "Review at next checkpoint"].map((item) => <span key={item}>{item}</span>)}</div>
