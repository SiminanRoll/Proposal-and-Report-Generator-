...
function sectionLabel(value: PresentationSection): string {
  ...
  if (value === "lifecycle") return "Equipment age";
...
function WarrantyStatusBadge({ device, project }: { device: ReturnType<typeof reportableLifecycleDevices>[number]; project: Project }) {
  ...
  return <span className={`warranty-status warranty-status-${status}`}><b>{warrantyStatusLabel(status)}</b><small>{device.warrantyExpires || "Warranty date not listed"}</small></span>;
}
...
function ClientReportOverview({ project }: { project: Project }) {
  ...
  const scope = project.hipaa.enabled
    ? "Security, equipment age, HIPAA readiness, and next-step priorities."
    : "Security, equipment age, infrastructure health, and next-step priorities.";
  return (
    <div className="presentation-health-cover">
      <div className="health-cover-main">
        ...
        <article className={`overall-health-score ${scoreTone(scores.overall)}`} style={{ "--score-value": scores.overall } as CSSProperties}>
          ...
          <small>{scores.provisional ? `${hipaa.notYetAssessedCount} HIPAA question${hipaa.notYetAssessedCount === 1 ? " remains" : "s remain"} unanswered, so this score will update as the assessment is completed.` : "A combined view of security protection, equipment age, and readiness findings."}</small>
        </article>
        <div className={`health-score-card-grid ${project.hipaa.enabled ? "" : "without-hipaa"}`}>
          ...
          <HealthStatusCard status={scoreLabel(scores.network)} label="Network & equipment age" detail={`${lifecycle.current} healthy · ${healthPriorities} aging systems · critical systems weighted`} className="network" tone={scoreTone(scores.network)} />
...
function LifecyclePresentation({ project }: { project: Project }) {
  ...
  return (
    <div className={`presentation-section-layout message-${message.tone}`}>
      <div className="presentation-section-heading network-health-heading"><span className="presentation-kicker">Equipment age & replacement planning</span><h2>{message.title}</h2><p>{message.subtitle}</p></div>
      <div className="network-health-overview">
        <div className="lifecycle-health-score"><strong><AnimatedNumber value={lifecycle.healthyPercentage} delay={180} suffix="%" /></strong><span>currently healthy</span><small><AnimatedNumber value={lifecycle.current} delay={300} /> of <AnimatedNumber value={lifecycle.assessed} delay={360} /> age-assessed physical assets are healthy{lifecycle.unknown ? <> · <AnimatedNumber value={lifecycle.unknown} delay={390} /> need age data</> : null}</small></div>
        <div className="lifecycle-story">
          ...
          <div className="lifecycle-legend"><span className="current"><b><AnimatedNumber value={lifecycle.current} delay={430} /></b> Healthy now</span><span className="due-soon"><b><AnimatedNumber value={lifecycle.dueSoon} delay={500} /></b> Plan soon</span><span className="overdue"><b><AnimatedNumber value={lifecycle.overdue} delay={570} /></b> Replace now</span><span className="unknown"><b><AnimatedNumber value={lifecycle.unknown} delay={640} /></b> Age to verify</span></div>
        </div>
      </div>
...
function DeviceDetailPresentation({ project }: { project: Project }) {
  ...
  const filterLabel = filter === "all" ? "all assets" : filter === "current" ? "healthy assets" : filter === "due-soon" ? "plan-soon assets" : filter === "overdue" ? "replace-now assets" : filter === "unknown" ? "assets needing age data" : filter === "os" ? osSupport.attention ? "operating-system concerns" : "assets with reported operating systems" : storage.attention ? "storage-attention assets" : "assets with reported storage";
  const cards = [
...
function PlanPresentation({ project, onUpdate }: { project: Project; onUpdate: (project: Project) => void }) {
  ...
  const headline = noActionNeeded ? "No immediate technology project needed" : agreedPlan ? agreedDecisions ? "Agreed technology roadmap" : "Agreed next step" : hasHardwareActions ? approach.title : hasActionItems ? "What should happen next" : approach.title;
  const intro = noActionNeeded
    ? "Your technology environment is currently in good health. No immediate hardware or technology project is needed. Routine administrative or compliance follow-up identified in the report can still be addressed without creating a major project."
    : agreedPlan
  ...
  return <div className={`presentation-section-layout client-action-plan ${hasActionItems ? "action-mode" : "healthy-mode"}`}>
    <div className="planning-hero-grid">
      ...
      {noActionNeeded ? <section className="planning-consultation-banner">
        <div><span className="presentation-kicker">Review outcome</span><h3>No immediate technology project needed</h3><p>Continue normal monitoring, maintenance, security protection, and support, address routine follow-up identified in the report, then revisit the environment at the next scheduled review.</p></div>
      </section> : agreedPlan ? <section className="planning-consultation-banner agreed-plan-banner">
      ...
      </section> : hasHardwareActions && (approach.mode === "onsite-project" || approach.mode === "remote-estimate") ? <OnsitePlanningScheduler
        project={project}
        onUpdate={onUpdate}
    ...
    <div className={`planning-context-strip ${project.hipaa.enabled ? "with-hipaa" : ""}`}>
      ...
      <span className={lifecycle.overdue ? "risk" : "healthy"}><strong><AnimatedNumber value={lifecycle.overdue} delay={510} /></strong><b>Replace now</b></span>
      ...
      <span className={osSupport.attention ? "risk" : "healthy"}><strong><AnimatedNumber value={osSupport.attention} delay={620} /></strong><b>OS concerns</b></span>
...
function RecapPresentation({ project, onUpdate }: { project: Project; onUpdate: (project: Project) => void }) {
  ...
  const agreedPlan = hasAgreedReviewPlan(project.reviewOutcome);
  const agreedDecisions = agreedPlan && hasAgreedReviewDecisions(project.reviewOutcome);
  const agreedActions = agreedPlan ? clientReportPlanActions(project) : [];
  const noActionNeeded = isNoActionNeeded(project);
  const hasActionItems = !noActionNeeded && (agreedPlan || healthPriorities > 0 || osSupport.attention > 0 || securityFollowUps > 0 || hipaaFollowUps > 0);
  const appointment = noActionNeeded || approach.mode === "purchase-planning" ? null : scheduledPlanningAppointment(project);
  const canSchedulePlanning = !noActionNeeded && healthPriorities > 0 && !agreedPlan && (approach.mode === "onsite-project" || approach.mode === "remote-estimate");
  return <div className={`presentation-recap ${hasActionItems ? "action-mode" : "healthy-mode"}`}>
    <div className="recap-heading-row">
      <div><span className="presentation-kicker">Final recap</span><h2>Today&apos;s takeaways</h2><p>{noActionNeeded ? "No immediate hardware or technology project is needed. Continue the current support and monitoring cadence while addressing any routine administrative or compliance follow-up identified in the report." : agreedPlan ? project.reviewOutcome.meetingSummary || "The review findings and client decisions are documented in one agreed roadmap." : healthPriorities ? approach.intro : hasActionItems ? "Most of the environment is healthy. The items that need attention are documented, and the next conversation can focus on practical decisions." : "The environment reviewed is in a healthy position, with no immediate replacement or corrective action recommended from this report."}</p></div>
  ...
  return <section className="report-compact-inventory" aria-label="Hardware included in the report">
    ...
    {devices.length ? <div className="report-compact-inventory-list">
      ...
      {devices.map((device) => <div className={`report-compact-inventory-row lifecycle-${device.lifecycleStatus}`} key={`${device.type}-${device.name}-${device.serial}`}>
        ...
        <span>{device.type === "vm" ? "Host dependent" : device.age && device.age > 0 ? `${device.age} yr` : "Original ship date not listed"}</span>
  ...
  return <div className="report-single-pane">
    <div className="report-main-cards">
      ...
      <article className="report-plan-card">
        ...
        {noActionNeeded
          ? <div className="report-plan-list"><div><CheckIcon /><span><strong>No immediate technology project needed</strong><small>Continue normal monitoring, maintenance, security protection, and support, address routine follow-up identified in the report, then revisit the environment at the next scheduled review.</small></span></div></div>
  ...
  return <>
    ...
    {richClientReport ? <>
      ...
      <section className="generator-command-center outcome-command-center" aria-label="Generator controls">
        ...
        <div className="generator-command-group planning-mode-group"><span>2 · Planned next step</span><div className="planning-mode-toggle" role="group" aria-label="Recommended planning format"><button type="button" className={planningMode === "onsite-review" ? "active" : ""} aria-pressed={planningMode === "onsite-review"} onClick={() => onSetPlanningMode("onsite-review")}>Onsite review</button><button type="button" className={planningMode === "remote-consultation" ? "active" : ""} aria-pressed={planningMode === "remote-consultation"} onClick={() => onSetPlanningMode("remote-consultation")}>Remote consultation</button><button type="button" className={planningMode === "hourly-onsite-service" ? "active" : ""} aria-pressed={planningMode === "hourly-onsite-service"} onClick={() => onSetPlanningMode("hourly-onsite-service")}>Hourly onsite</button><button type="button" className={planningMode === "no-action-needed" ? "active" : ""} aria-pressed={planningMode === "no-action-needed"} onClick={() => onSetPlanningMode("no-action-needed")}>No immediate project</button></div></div>
