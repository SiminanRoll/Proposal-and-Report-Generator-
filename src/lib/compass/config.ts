...
export const COMPASS_CARD_SIGNAL_OPTIONS: Array<{ value: CompassCardSignal; label: string; description: string }> = [
  ...
  { value: "windows-10-active", label: "Active Windows 10 device", description: "Windows 10 device with no six-month stale check-in signal. Windows 8 / 8.1 are also treated as end of support by the shared OS-health engine." },
...
export const DEFAULT_COMPASS_CARDS: CompassCardDefinition[] = [
  ...
  },
  {
    ...
    description: "Five or more active Windows 10 devices. Windows 8 / 8.1 are also classified as end of support by the shared OS-health engine. Physical workstations, VMs, and classification-review devices count toward the threshold; only valid device types create replacement value.",
    accent: "cyan",
    icon: "windows",
    criteriaType: "signals",
    workflowRule: "",
  ...
  },
  {
    ...
    order: 6,
    title: "Reviews Due",
    countLabel: "clients with an overdue or missing account review",
    valueLabel: "estimated opportunity represented",
    description: "Clients whose account review is missing or older than the configured review interval. Review timing does not change the technical priority score.",
    accent: "blue",
    icon: "calendar",
    criteriaType: "workflow",
    workflowRule: "reviews-due",
    workflowMonths: 0,
    matchMode: "any",
    rules: [],
    sourceCardIds: [],
  ...
  },
  {
    ...
    order: 7,
    title: "Quote Needed",
    countLabel: "project-ready clients not yet quoted",
    valueLabel: "unquoted estimated opportunity represented",
    description: "Clients with a current technical project opportunity whose Quoted status is still blank. Quoted status does not change the technical priority score.",
    accent: "teal",
    icon: "compass",
    criteriaType: "workflow",
    workflowRule: "quote-needed",
    workflowMonths: 0,
    matchMode: "any",
    rules: [],
    sourceCardIds: [],
...
function normalizeCard(candidate: unknown, fallback: CompassCardDefinition | null, index: number): CompassCardDefinition | null {
  ...
  return {
    ...
    accent,
    icon,
    criteriaType: raw.criteriaType === "rollup" || raw.criteriaType === "workflow" ? raw.criteriaType : fallback?.criteriaType ?? "signals",
    workflowRule: raw.workflowRule === "reviews-due" || raw.workflowRule === "quote-needed" ? raw.workflowRule : fallback?.workflowRule ?? "",
    workflowMonths: Math.max(0, Math.round(finite(raw.workflowMonths, fallback?.workflowMonths ?? 0))),
...
function normalizeCards(value: unknown): CompassCardDefinition[] {
  ...
  for (const [index, candidate] of incoming.entries()) {
    ...
    const card = normalizeCard({ ...(candidate as object), id: customId, builtIn: false, criteriaType: "signals", workflowRule: "", workflowMonths: 0 }, null, DEFAULT_COMPASS_CARDS.length + index);
    if (card) normalized.push(card);
  }
