...
export function buildCompassGeneratorPrefill(dataset: CompassDataset, clientId: string, now = new Date(), config: CompassConfig = DEFAULT_COMPASS_CONFIG): CompassGeneratorPrefill | null {
  ...
  const facts: ExtractedFact[] = [
    ...
    fact({ key: "scalepad.os.unsupported", label: "Operating systems unsupported", value: osUnsupported, category: "lifecycle", confidence: "high", sourceFileId, evidence: "Windows 8 / 8.1, Windows 10, and unsupported server OS rules" }),
