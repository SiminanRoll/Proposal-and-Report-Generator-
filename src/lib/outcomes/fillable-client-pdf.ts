...
export function buildFillablePdfBytes(pages: PdfRasterPage[], title: string, layout: PdfPageLayout = LANDSCAPE_LAYOUT): Uint8Array {
  ...
  for (let pageIndex = 0; pageIndex < pages.length; pageIndex += 1) {
    ...
    for (const link of input.links ?? []) {
      ...
      store.set(linkReference, bytes(`<< /Type /Annot /Subtype /Link /Rect [${x1.toFixed(2)} ${y1.toFixed(2)} ${x2.toFixed(2)} ${y2.toFixed(2)}] /P ${pageReference} 0 R /Border [0 0 0] /A << /S /URI /URI ${pdfString(link.url)} >> >>`));
    }
...
function captureLinks(page: HTMLElement, layout: PdfPageLayout): PdfLinkDefinition[] {
  const pageRect = page.getBoundingClientRect();
  if (!pageRect.width || !pageRect.height) return [];
  return Array.from(page.querySelectorAll<HTMLElement>("a[href], [data-pdf-link]")).flatMap((element) => {
    const raw = element.dataset.pdfLink?.trim() || element.getAttribute("href")?.trim() || "";
    if (!raw) return [];
    let url = "";
    try {
