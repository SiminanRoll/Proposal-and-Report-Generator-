 ...
 * does not already contain it. The final copy sanitizer removes legacy blanket
 * health claims, then the website-link pass adds a few contextual Advantage
 * resources whose coordinates are preserved as native PDF link annotations.
 */
export async function downloadFillableClientPdf(html: string, documentTitle: string): Promise<void> {
  ...
  const sanitizedHtml = sanitizeClientPdfCopy(inventoryHtml);
