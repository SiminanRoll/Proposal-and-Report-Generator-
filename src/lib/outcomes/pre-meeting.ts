...
function preMeetingLabel(project: Project): string {
  return project.hipaa.enabled ? "Pre-meeting packet" : "Pre-meeting overview";
}

export function preMeetingDocumentTitle(project: Project): string {
  const clientName = project.client.name.trim();
  const documentName = project.hipaa.enabled ? "Technology Review Pre-Meeting Packet" : "Technology Review Overview";
...
function preparationPanel(project: Project): string {
  if (project.hipaa.enabled) {
    return `<section class="panel"><h2>Helpful information to have available</h2><ul><li>Who is responsible for HIPAA policies and employee training</li><li>Whether risk assessments are performed and documented</li><li>How access is added or removed when team members change</li><li>Whether backup recovery is periodically tested</li><li>How security incidents are documented and handled</li></ul></section><section class="panel"><h2>Who should attend?</h2><p>Ideally, someone familiar with the practice’s operations, technology, or HIPAA procedures should join the conversation. This may be the doctor, practice manager, office manager, or HIPAA coordinator.</p></section>`;
  }
...
export function preMeetingOverviewHtml(project: Project): string {
  ...
  const reassurance = hipaaEnabled
    ? `<div><strong>You can complete the included questions now—or wait.</strong><span>No advance research is required. Leave anything blank that you would rather discuss together during the meeting.</span></div>`
...
export function preMeetingEmailDraft(project: Project): { recipient: string; subject: string; body: string; mailto: string } {
  ...
  const preparationCopy = project.hipaa.enabled
    ? "The attached packet also includes the HIPAA technology questions we’ll discuss. You’re welcome to review or complete them in advance, or leave them blank and we’ll work through them together during the meeting. There is no need to research every answer beforehand."
  ...
  const attachmentName = project.hipaa.enabled ? "pre-meeting packet" : "short overview";
