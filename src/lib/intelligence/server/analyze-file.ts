...
function parseRft(buffer: Buffer, fileId: string): FileAnalysis {
...
export async function analyzeFile(input: {
  ...
  if (["jpg", "jpeg", "png", "webp"].includes(extension) || input.mimeType.startsWith("image/")) {
    return {
      ...
      facts: [fact({ key: `photo.${input.fileId}.metadata`, label: "Office photo", value: `${Math.max(1, Math.round(input.buffer.length / 1024))} KB`, category: "operations", confidence: "high", sourceFileId: input.fileId, evidence: input.fileName })],
      findingCandidates: [],
